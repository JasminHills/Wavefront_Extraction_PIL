...
class psf_dataset(Dataset):
    ...
    def __getitem__(self, id):
        ...
        image = sample_hdu[2].data.astype(np.float32)
...
class Normalize(object):
    def __call__(self, sample):
        ...
        image[0] = minmax(np.log(image[0]))
#         image[1] = minmax(np.sqrt(image[1]))
        
#         phase = (phase/900.)*2*np.pi
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        return {'phase': torch.from_numpy(phase), 'image': torch.from_numpy(image[0])}
...
def splitDataLoader(dataset, split=[0.9, 0.1], batch_size=32, random_seed=None, shuffle=True):
    ...
    train_dataloader = DataLoader(dataset, batch_size=batch_size, num_workers=4, sampler=train_sampler, pin_memory=True)
    val_dataloader = DataLoader(dataset, batch_size=batch_size, num_workers=4, sampler=val_sampler, pin_memory=True)
